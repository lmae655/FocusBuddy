...
MOTIVATIONAL_MESSAGES = {"Studying": ["Great focus session! Keep it up! 📚", "You're doing amazing! Stay focused! 💪", "Learning is a superpower! Keep going! 🚀", "Your future self will thank you! 📖", "Excellence is earned through dedication! ⭐",], "Gaming": ["Having fun? Don't forget to take breaks! 🎮", "Remember to rest your eyes soon! 👀", "Balance is key! Consider a study session next! 🎯", "Gaming is cool, but health comes first! 🏥",], "Entertainment": ["Enjoying yourself? That's great! 🎬", "Don't forget your daily goals! 🎯", "Time for a break? Your eyes need it! 👁️", "You deserve entertainment, but balance matters! ⚖️",], "Other": ["Keep up the good work! 💯", "You're making progress! 🌟", "Stay consistent! 🎯",]}
...
THEMES = {"Neon Purple": {"bg": "#0a0e27", "primary": "#b721ff", "secondary": "#ff006e", "accent": "#00d9ff", "card_bg": "#1a1f3a", "text": "#ffffff", "text_light": "#b0b0b0", "success": "#00ff88", "warning": "#ffa500"}, "Ocean Blue": {"bg": "#0f1419", "primary": "#00b4d8", "secondary": "#0096c7", "accent": "#00d9ff", "card_bg": "#1a2332", "text": "#ffffff", "text_light": "#a8dadc", "success": "#06d6a0", "warning": "#fb5607"}, "Forest Green": {"bg": "#0b1929", "primary": "#2ecc71", "secondary": "#27ae60", "accent": "#1abc9c", "card_bg": "#1a3a2a", "text": "#ecf0f1", "text_light": "#95a5a6", "success": "#f39c12", "warning": "#e74c3c"}, "Sunset Glow": {"bg": "#1a0f2e", "primary": "#ff6b6b", "secondary": "#ff8e3c", "accent": "#ffd93d", "card_bg": "#2d1b3d", "text": "#ffffff", "text_light": "#d4a5a5", "success": "#6bcf7f", "warning": "#ff6348"}, "Cyber Dark": {"bg": "#0d0221", "primary": "#3a86ff", "secondary": "#8338ec", "accent": "#fb5607", "card_bg": "#1a0033", "text": "#ffffff", "text_light": "#c0c0c0", "success": "#06ffa5", "warning": "#ff006e"}, "Vibrant Pink": {"bg": "#2d0a3d", "primary": "#ff1493", "secondary": "#ff69b4", "accent": "#ffb6d9", "card_bg": "#3d1a4d", "text": "#ffffff", "text_light": "#ffb6d9", "success": "#00ff88", "warning": "#ffa500"}, "Cool Mint": {"bg": "#0d2e2e", "primary": "#00d9a3", "secondary": "#00b8a9", "accent": "#8ff7eb", "card_bg": "#1a4d4d", "text": "#ffffff", "text_light": "#a0e4cb", "success": "#00d9a3", "warning": "#ff6b6b"}, "Deep Purple": {"bg": "#1a0033", "primary": "#8b00ff", "secondary": "#6600cc", "accent": "#c77dff", "card_bg": "#2d0052", "text": "#ffffff", "text_light": "#c77dff", "success": "#06ffa5", "warning": "#ff006e"}}
class FocusBuddyApp:
    ...
    def setup_styles(self):
        ...
        style.configure("TProgressbar", background=self.theme["primary"], troughcolor=self.theme["card_bg"], bordercolor=self.theme["accent"], lightcolor=self.theme["primary"], darkcolor=self.theme["primary"])
        style.configure("TCombobox", fieldbackground=self.theme["card_bg"], background=self.theme["primary"], foreground=self.theme["text"])
    def load_data(self):
    ...
    def create_ui(self):
        ...
        title_label = tk.Label(header_frame, text="⚡ FocusBuddy", font=("Arial", 28, "bold"), bg=self.theme["primary"], fg=self.theme["text"])
        title_label.pack(pady=(10, 0))
        subtitle_label = tk.Label(header_frame, text="Smart Screen-Time Management for Students", font=("Arial", 10), bg=self.theme["primary"], fg=self.theme["text_light"])
        ...
        tk.Label(theme_selector_frame, text="Theme:", font=("Arial", 9, "bold"), bg=self.theme["primary"], fg=self.theme["text"]).pack(side=tk.LEFT, padx=5)
        theme_combo = ttk.Combobox(theme_selector_frame, values=list(THEMES.keys()), state="readonly", width=15, font=("Arial", 9))
    ...
    def create_card(self, parent, title, icon=""):
        ...
        header_label = tk.Label(header, text=f"{icon} {title}", font=("Arial", 12, "bold"), bg=self.theme["primary"], fg=self.theme["text"])
    ...
    def create_mega_dashboard(self, parent):
        ...
        header.pack(fill=tk.X, padx=0, pady=0)
        header.pack_propagate(False)
        header_label = tk.Label(header, text="🎯 TODAY'S PERFORMANCE", font=("Arial", 14, "bold"), bg=self.theme["accent"], fg=self.theme["bg"])
        ...
        tk.Label(st_frame, text="Screen Time", font=("Arial", 9, "bold"), bg=self.theme["card_bg"], fg=self.theme["text_light"]).pack()
        self.screen_time_label = tk.Label(st_frame, text="0h 0m", font=("Arial", 24, "bold"), bg=self.theme["card_bg"], fg=self.theme["secondary"])
        ...
        tk.Label(dg_frame, text="Daily Goal", font=("Arial", 9, "bold"), bg=self.theme["card_bg"], fg=self.theme["text_light"]).pack()
        self.goal_label = tk.Label(dg_frame, text="8h 0m", font=("Arial", 24, "bold"), bg=self.theme["card_bg"], fg=self.theme["primary"])
        ...
        tk.Label(br_frame, text="Breaks Taken", font=("Arial", 9, "bold"), bg=self.theme["card_bg"], fg=self.theme["text_light"]).pack()
        self.breaks_label = tk.Label(br_frame, text="0", font=("Arial", 24, "bold"), bg=self.theme["card_bg"], fg=self.theme["success"])
        ...
        tk.Label(progress_frame, text="Progress to Goal", font=("Arial", 11, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"]).pack(anchor=tk.W, pady=(0, 8))
        self.progress_var = tk.DoubleVar()
        progress_bar = ttk.Progressbar(progress_frame, variable=self.progress_var, maximum=100, mode='determinate')
        progress_bar.pack(fill=tk.X, pady=5)
        self.progress_label = tk.Label(progress_frame, text="0% used", font=("Arial", 9, "bold"), bg=self.theme["card_bg"], fg=self.theme["accent"])
        self.progress_label.pack(anchor=tk.W)
        self.motivational_label = tk.Label(dash, text="Keep crushing your goals! 💪", font=("Arial", 11, "italic"), bg=self.theme["card_bg"], fg=self.theme["accent"], wraplength=600)
    ...
    def create_activity_section(self, parent):
        ...
        for i, activity in enumerate(activities):
            ...
            rb = tk.Radiobutton(rb_frame, text=activity, variable=self.current_activity, value=activity, font=("Arial", 11, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"], activebackground=self.theme["card_bg"], activeforeground=self.theme["accent"], selectcolor=self.theme["primary"], command=self.update_dashboard)
    ...
    def create_tracking_section(self, parent):
        ...
        tk.Label(quick_frame, text="Quick Add:", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"]).pack(side=tk.LEFT, padx=5)
        for minutes in [15, 30, 60]:
            btn = tk.Button(quick_frame, text=f"+ {minutes}m", command=lambda m=minutes: self.add_screen_time(m), font=("Arial", 10, "bold"), bg=self.theme["secondary"], fg=self.theme["text"], padx=12, pady=6, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
            btn.pack(side=tk.LEFT, padx=5)
        custom_frame = tk.Frame(content, bg=self.theme["card_bg"])
        custom_frame.pack(fill=tk.X, pady=10)
        tk.Label(custom_frame, text="Custom Time (min):", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"]).pack(side=tk.LEFT, padx=5)
        self.custom_entry = tk.Entry(custom_frame, width=8, font=("Arial", 10), bg=self.theme["card_bg"], fg=self.theme["text"], insertbackground=self.theme["accent"], relief=tk.FLAT, bd=2)
        self.custom_entry.pack(side=tk.LEFT, padx=5)
        add_btn = tk.Button(custom_frame, text="Add", command=self.add_custom_time, font=("Arial", 9, "bold"), bg=self.theme["primary"], fg=self.theme["text"], padx=15, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
        add_btn.pack(side=tk.LEFT, padx=5)
        timer_frame = tk.Frame(content, bg=self.theme["card_bg"])
        timer_frame.pack(fill=tk.X, pady=15)
        tk.Label(timer_frame, text="Live Timer:", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"]).pack(side=tk.LEFT, padx=5)
        self.timer_label = tk.Label(timer_frame, text="0h 0m 0s", font=("Arial", 14, "bold"), bg=self.theme["card_bg"], fg=self.theme["accent"])
        self.timer_label.pack(side=tk.LEFT, padx=10)
        button_frame = tk.Frame(timer_frame, bg=self.theme["card_bg"])
        button_frame.pack(side=tk.LEFT, padx=5)
        self.start_btn = tk.Button(button_frame, text="▶ Start Session", command=self.start_session, font=("Arial", 9, "bold"), bg=self.theme["success"], fg=self.theme["bg"], padx=12, pady=5, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
        self.start_btn.pack(side=tk.LEFT, padx=2)
        self.stop_btn = tk.Button(button_frame, text="⏹ Stop", command=self.stop_session, font=("Arial", 9, "bold"), bg=self.theme["warning"], fg=self.theme["bg"], padx=12, pady=5, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"], state=tk.DISABLED)
    ...
    def create_stats_section(self, parent):
        ...
        for idx, activity in enumerate(["Studying", "Gaming", "Entertainment", "Other"]):
            ...
            tk.Label(act_frame, text=f"{activity}:", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"], width=15, anchor=tk.W).pack(side=tk.LEFT, padx=5)
            self.stats_labels[activity] = tk.Label(act_frame, text="0h 0m", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=colors[idx])
    ...
    def create_controls_section(self, parent):
        ...
        tk.Label(goal_frame, text="Daily Goal (hours):", font=("Arial", 10, "bold"), bg=self.theme["card_bg"], fg=self.theme["text"]).pack(side=tk.LEFT, padx=5)
        self.goal_entry = tk.Entry(goal_frame, width=5, font=("Arial", 10), bg=self.theme["card_bg"], fg=self.theme["text"], insertbackground=self.theme["accent"], relief=tk.FLAT, bd=2)
        self.goal_entry.pack(side=tk.LEFT, padx=5)
        self.goal_entry.insert(0, str(self.session_data['daily_goal'] // 60))
        set_goal_btn = tk.Button(goal_frame, text="Set Goal", command=self.set_daily_goal, font=("Arial", 9, "bold"), bg=self.theme["primary"], fg=self.theme["text"], padx=15, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
        set_goal_btn.pack(side=tk.LEFT, padx=5)
        btn_frame = tk.Frame(content, bg=self.theme["card_bg"])
        btn_frame.pack(fill=tk.X, pady=15)
        break_btn = tk.Button(btn_frame, text="☕ Take a Break", command=self.take_break, font=("Arial", 10, "bold"), bg=self.theme["success"], fg=self.theme["bg"], padx=20, pady=8, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
        break_btn.pack(side=tk.LEFT, padx=5)
        reset_btn = tk.Button(btn_frame, text="🔄 Reset Daily Data", command=self.reset_daily_data, font=("Arial", 10, "bold"), bg=self.theme["warning"], fg=self.theme["bg"], padx=20, pady=8, relief=tk.FLAT, cursor="hand2", activebackground=self.theme["accent"])
